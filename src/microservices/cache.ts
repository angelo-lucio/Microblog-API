...
try {    await redis.ping()
    ...
    logger.fatal({ err: error, module: 'Redis'}, 'Failed to connect to Redis cache' )
    process.exit(1)
      }
    }
...
export const getPosts = async (userId?: number) => {
  if (!CACHE_ACTIVE) {
    logger.warn('Cache is disabled, fetching posts from database')
  ...
  if (cachedData) {
    logger.debug('Posts retrieved from cache'); // If found in cache, return it
    return JSON.parse(cachedData) as Posts;
  }
  logger.debug('Posts not found in cache, fetching from database...') // If not found in cache, fetch from database
...
export const invalidatePostsCache = async () => {
  ...
  logger.info('Cache invalidated') // Invalidate the posts cache by deleting the cached data for all posts
    }
